...
object RecoverableNetworkWordCount {
  ...
  def createContext(ip: String, port: Int, outputPath: String, checkpointDirectory: String): StreamingContext = {
    // If you don't see this printed, that means the StreamingContext has been loaded
    ...
    // Create a socket stream on target ip:port and count the words in input stream
    // of \n delimited text (eg. generated by 'nc')
    val lines = ssc.socketTextStream(ip, port).checkpoint(Seconds(10))
    ...
    wordCounts.foreachRDD { (rdd: RDD[(String, Int)], time: Time) =>
      ...
      val droppedWordsCounter = DroppedWordsCount.getInstance(rdd.sparkContext)
