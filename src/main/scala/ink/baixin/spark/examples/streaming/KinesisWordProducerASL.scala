...
/**
  ...
  *   <words-per-record> is the number of words per records to put onto the stream
  *
  * Example:
...
object KinesisWordProducerASL {
  ...
  def generate(stream: String,
    ...
    val randomWords = List("Spark", "you", "are", "my", "son")
    ...
    // Iterate and put records onto the stream per the given recordsPerSec and wordsPerRecord
    for (i <- 1 to 10) {
    ...
    // Conver the totals to (index, total) tuple
